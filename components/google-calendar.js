export const GoogleCalendar = (() => {
  ...
  function loadTokens() {
    return new Promise((resolve) =>
      chrome.storage.local.get("calTokens", ({ calTokens }) =>
        resolve(calTokens || null),
      ),
    );
  }
  ...
  async function checkAndBlock() {
    ...
    try {
      ...
      if (active) {
        const { blockSites } = await new Promise((r) =>
          chrome.storage.local.get("blockSites", r),
        );
        const endTime = new Date(
          active.end?.dateTime || active.end?.date,
        ).getTime();
        chrome.storage.local.set({
          focusMode: {
            active: true,
            ...
            eventTitle: active.summary || "Meeting",
          },
        });
        chrome.notifications?.create("cal_block_" + Date.now(), {
          ...
          message: "Distracting sites blocked for this event.",
          priority: 1,
        });
      } else {
        const { focusMode } = await new Promise((r) =>
  ...
  function renderCalendarPanel(container) {
    chrome.storage.local.get(
      ...
      ({ calendarBlockEnabled, calendarConnected, calUserEmail }) => {
        ...
        container
          .querySelector("#calAutoBlockToggle")
          ?.addEventListener("change", (e) => {
            chrome.storage.local.set({
              calendarBlockEnabled: e.target.checked,
            });
            if (e.target.checked) checkAndBlock();
          });

        container
          .querySelector("#calConnectBtn")
          ?.addEventListener("click", async () => {
            ...
            try {
              if (
                ...
                throw new Error(
                  "Set your Google CLIENT_ID in google-calendar.js first",
                );
              }
        ...
        container
          .querySelector("#calDisconnectBtn")
          ?.addEventListener("click", async () => {
            await signOut();
            renderCalendarPanel(container);
          });
          ...
          getValidToken()
            .then((token) => {
              if (token) loadTodayEvents(container, token);
                ...
                clearTokens();
                chrome.storage.local.set({ calendarConnected: false });
                renderCalendarPanel(container);
              }
            })
            .catch(() => {});
  ...
  async function loadTodayEvents(container, token) {
    ...
    try {
      const events = await fetchTodayEvents(token);
      if (!events.length) {
        listEl.innerHTML = `<p class="card-hint">No events today.</p>`;
        return;
      }
      listEl.innerHTML = `
