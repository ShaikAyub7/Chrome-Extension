export const Dashboard = (() => {
    ...
    return new Promise((resolve) =>
      chrome.storage.local.get(["widgetConfig", "siteOrder"], (data) => {
        ...
        const merged = WIDGET_DEFS.map((def) => {
          ...
          return {
            ...def,
            enabled: found ? found.enabled : def.id !== "mini-clock",
          };
        });
  ...
  function saveWidgetConfig(config) {
    chrome.storage.local.set({
      widgetConfig: config.map(({ id, enabled }) => ({ id, enabled })),
    });
  }
  ...
  function stampDomains(container) {
    let stamped = false;
    container
      .querySelectorAll(".site-usage-box:not([data-domain])")
      .forEach((box) => {
        const anchor =
          box.querySelector(".site-domain a") ||
          box.querySelector("a[href^='https://']");
        if (!anchor) return;
        try {
  ...
  function enableDragSort(container, onReorder) {
    ...
    container.addEventListener("drop", (e) => {
      ...
      container
        .querySelectorAll(".drag-over")
        .forEach((el) => el.classList.remove("drag-over"));
      const newOrder = [...container.querySelectorAll("[data-domain]")].map(
  ...
  function getDragAfterElement(container, y) {
    const els = [
      ...container.querySelectorAll("[data-domain]:not(.drag-active)"),
    ];
    let closest = null,
  ...
  function applySiteOrder(container) {
    ...
    container.querySelectorAll("[data-domain]").forEach((el) => {
      map[el.dataset.domain] = el;
    });
    const fragment = document.createDocumentFragment();
    siteOrder.forEach((d) => {
      if (map[d]) fragment.appendChild(map[d]);
    });
    Object.entries(map).forEach(([d, el]) => {
      if (!siteOrder.includes(d)) fragment.appendChild(el);
    });
